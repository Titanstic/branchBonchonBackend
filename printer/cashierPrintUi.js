...
const cashierPrintSlipBuffer = async (
  ...
  data.forEach((each) => {
    ...
    if (each.combo_menu_items) {
      const comboMenuItems =
        typeof each.combo_menu_items == "string"
          ? JSON.parse(each.combo_menu_items)
          : each.combo_menu_items;
  ...
  let canvasHeight =
    originalHeight +
    data.length * 50 +
    flavourTypeLength * 30 +
    containerDataLength * 25 +
    discountDataLength * 25;
  ...
  buyItemUi(ctx, canvas, data, headerHeight, firstLineH, transitionId);
  paymentDetailUi(
    ctx,
    canvas,
    sub_total_amount,
...
const slipHeightData = (data, point) => {
  ...
  const invoiceH = tableCashierH + 50;
  const informationLineH = invoiceH + 30;

  const headerHeight = informationLineH + 50;
...
const buyItemUi = (
  ...
  data.forEach((productItem, index) => {
    ...
    ctx.textAlign = "right";
    ctx.fillText(
      productItem.quantity.toLocaleString("en-US"),
      canvas.width - 150,
      flavourYPos
    );
    ctx.fillText(
    ...
    if (productItem.combo_menu_items) {
      const comboMenuItems =
        typeof productItem.combo_menu_items == "string"
          ? JSON.parse(productItem.combo_menu_items)
          : productItem.combo_menu_items;
    ...
    if (productItem.container_charges && productItem.container_charges > 0) {
      ...
      ctx.textAlign = "right";
      ctx.fillText(
        productItem.container_charges.toLocaleString("en-US"),
        canvas.width - 10,
        flavourYPos
      );
    }
...
const paymentDetailUi = (
  ...
  ctx.textAlign = "right";
  ctx.fillText(
    sub_total_amount.toLocaleString("en-US"),
    canvas.width - 10,
    customLastByItemLineH + 30
  );

  //discount
  ctx.textAlign = "start";
  ctx.fillText(`Discount`, 20, customLastByItemLineH + 60);
  ctx.textAlign = "right";
  ctx.fillText(
    `- ${discount_amount.toLocaleString("en-US")}`,
    canvas.width - 10,
    customLastByItemLineH + 60
  );

  ctx.textAlign = "start";
  ctx.fillText(`Tax (5%)`, 20, customLastByItemLineH + 90);
  ctx.textAlign = "right";
  ctx.fillText(
    (tax_amount + add_on + inclusive + service_charge_amount).toLocaleString(
      "en-US"
    ),
    canvas.width - 10,
    customLastByItemLineH + 90
  );
  ...
  ctx.textAlign = "right";
  ctx.fillText(
    `${grand_total_amount.toLocaleString("en-US")}`,
    canvas.width - 10,
    customLastByItemLineH + 150
  );
  ...
  ctx.textAlign = "right";
  ctx.fillText(
    `${cash_back.toLocaleString("en-US")}`,
    canvas.width - 10,
    customLastByItemLineH + 210
  );
...
const qrUi = async (transitionId, point, ctx, canvas, grand_total_amount) => {
  ...
  ctx.textAlign = "center";
  ctx.fillText(
    "Scan this QR Code to Collect Points.",
    canvas.width / 2,
    customLastLineH + qrHeight + 40
  );

  customQrLineH = customLastLineH + qrHeight + 50;
};

const footerUi = (ctx, canvas, point) => {
  console.log("cashierPrintUi [footerUi]: print footerUi");
  const customThankH = point ? customQrLineH + 20 : customLastLineH + 20;
  //thank you
  ctx.textAlign = "center";
  ctx.fillText(
    "Thank You for visiting Bonchon.....",
    canvas.width / 2,
    customThankH
  );
  ctx.fillText("See You  Again!!", canvas.width / 2, customThankH + 20);
};
